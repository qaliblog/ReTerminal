    ...
    fun runGitCommand(path: String, command: String, onDone: (Int, String) -> Unit) {
        scope.launch(Dispatchers.IO) {
            try {
                ...
                val result = AgentOrchestrator.MainShell.execInMainSession(mainActivityActivity, path, cmd, 60_000L)
                val out = result.first
                val exit = result.second
                val body = out.lineSequence().filter { it.startsWith("EXIT_CODE=").not() }.joinToString("\n").trim()
