...
if (exit != 0) {
            ...
            "write_file" -> {
                // Do not inject templates. If LLM did not provide a concrete write/apply_changes, discover current state first.
                val t = proposed.type.lowercase().trim()
                ...
                val target = when {
                    ...
                    desc.contains("css") -> "static/style.css"
                    desc.contains("javascript") || desc.contains("js") -> "static/app.js"
                    desc.contains("python") -> "main.py"
                    else -> wd
                }
                val file = resolvePath(target)
                return if (file.exists() && file.isFile) {
                    ToolCall("read_file", JSONObject().put("path", target).put("max_bytes", 200_000))
                } else {
                    ToolCall("create_file", JSONObject().put("path", target))
                }
            }
